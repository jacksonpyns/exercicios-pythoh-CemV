...
# dicionário de exemplo, com alguns dados padrões
agenda = {
    "Pessoa 1":{
...
def agenda_para_texto(**agenda_completa):
    """Recebe um dicionário de dicionários com a agenda que será exibida
    e
    retorna uma string com este dicionário formatado"""
    formato_texto = ""
    for nome_contato, formas_contato in agenda_completa.items():
        formato_texto = (f"{formato_texto}"
                         f"{contato_para_texto(nome_contato, **formas_contato)}\n")
        formato_texto = f"{formato_texto}--------------------------\n"
    return formato_texto
...
def altera_nome_contato(agenda_original:dict, nome_original:str, nome_atualizado:str):
    """Recebe a agenda original em forma de dicionário, o nome_original e o nome_atualizado
    em forma de string.
    Busca o nome original no dicionário e retorna False se não encontrar.
    Retorna True se encontrar o nome original no dicionário e fizer a
    exclusão do contato antigo e inclusão do novo"""
...
def altera_forma_contato(lista_contatos:list, valor_antigo:str, novo_valor:str):
    """Recebe uma list lista_contatos, o valor antigo que será
    substituído e o novo valor.
...
def exclui_contato(agenda:dict, nome_contato:str):
    """Recebe uma agenda completa como dicionário e o nome do contato
    como string.
    Caso o nome do contato não esteja nas chaves do dicionário, retornará
     False
    Caso o nome do contato esteja nas chaves, o registro correpondente
     será removido e retornará True"""
...
def inclui_contato(agenda:dict, nome_contato:str, **formas_contato):
     ...
     como string e as formas de contato
     em um dicionário como **kwargs.
     Não é feita nenhuma verificação, portanto se já existir um contato com
...
#Função para inserir uma nova forma de contato
def inclui_forma_contato(formas_contato:dict, forma_incluida:str, valor_incluido:str):
    ...
    Caso a forma de contato ainda não exista e estiver presente na tupla
     de formas de contatos suportados
     será incluída e o novo valor será incluído em uma lista, retornando
     True.
    Caso a forma de contato ainda não exista e não estiver presente na
     tupla de formas de contato suportados,
     retornará False."""
...
#onde o USUARIO vai incluir o contato
def usuario_inclui_contato(agenda:dict):
    nome = input("Informa o nome do novo contato que será inserido na agenda: ")
    dicionario_formas = {}
    for forma in contatos_suportados:
        ...
        while "S" in resposta.upper():
            ...
            resposta = input(f"Deseja inserir outro {forma} para"
                             f"{nome.capitalize()}?\nSIM ou NÃO -> ")
...
#onde o USUARIO vai incluir uma forma de contato
def usuario_inclui_forma_contato(agenda:dict):
    ...
    if nome in agenda.keys():
        ...
        if forma_incluida in contatos_suportados:
            ...
            print("A forma de contato indicada não é suportada pelo sistema. A agenda não"
                  "foi alterada.")
    else:
        print("O contato informado não existe na agenda. Não forma feitas alterações.")

#onde o USUARIO exclui um contato
def usuario_exclui_contato(agenda:dict):
    nome = input("Informe o nome do contato que deseja excluir: ")
    if exclui_contato(agenda, nome): #como essa função retorna True ou False pode-se usar junnto com if
        print("Usuário excluido com sucesso!")
    else:
        print("Nome do usuário não localizado na agenda. Não forma feitas alterações.")

#onde o USUARIO altere o nome do contato
...
#onde o USUARIO altera a forma do contato
def usuario_altera_forma_contato(agenda:dict):
    ...
    if nome in agenda.keys():
        ...
        if forma_incluida in contatos_suportados:
            ...
            if altera_forma_contato(agenda[nome][forma_incluida],
                                    valor_antigo, nova_valor):
                print("Contato alterado com sucesso!")
            else:
                print("Ocorreu um erro durante a alteração do contato. A"
                      "agenda não foi alterada.")
        else:
            print(f"{forma_incluida} não é uma forma de contato suportada "
                  f"pelo sistema. A agenda não foi alterada.")
    else:
        print(f"O contato {nome} não está na agenda. A agenda não foi alterada.")

#onde o USUARIO define qual contato ele quer mostrar os dados
...
#onde o USUARIO interaja para escolher a opcao
def exibe_menu():
    print("\n\n")
